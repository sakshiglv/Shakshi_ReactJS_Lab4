...
function ShowData() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            data.map((each) => 
                each.payeeName === "Rahul" ? (rahulSpentAmount = rahulSpentAmount + each.price) 
                : (rameshSpentAmount = rameshSpentAmount + each.price));
    ...
    return (
        <>
            ...
            {showForm && (
                <div className="form">
                    <AddExpense onTrue={success} onClose={cancel}></AddExpense>
                </div>
            )}
            ...
            <span className="use-inline total rahul">{rahulSpent}</span> <br></br>
            <div className="use-inline">Ramesh Spent: </div>
            <span className="use-inline total ramesh">{rameshSpent}</span> <br></br>
